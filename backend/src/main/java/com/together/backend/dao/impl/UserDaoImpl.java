...
public class UserDaoImpl implements UserDao {
        ...
        try {
            ...
            if (data != null && id != null && !id.isEmpty()) {
                ...
                ApiFuture<WriteResult> future = reference.update(
                        ...
                        "secondName",
                        (data.getSecondName() != null) ? data.getSecondName() : bufferUser.getSecondName(),
                        "surname", (data.getSurname() != null) ? data.getSurname() : bufferUser.getSurname(),
                        "contactPhone",
                        (data.getContactPhone() != null) ? data.getContactPhone() : bufferUser.getContactPhone(),
                        "role", (data.getRole() != null) ? data.getRole() : bufferUser.getRole(),
                        "password", (data.getPassword() != null) ? data.getPassword() : bufferUser.getRole(),
                        "email", (data.getEmail() != null) ? data.getEmail() : bufferUser.getEmail());

                operationResponse = new OperationResponse<>("[UPDATE] User updated at: " + future.get().getUpdateTime(),
                        getUserById(id).getEntity());
        ...
        try {
            ...
            if (id != null && !id.isEmpty()) {
                ...
                operationResponse = new OperationResponse<>(
                        "[Deleted] User deleted at: " + future.get().getUpdateTime(), null);
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public OperationResponse<BasicUser> getUserByEmail(@Nonnull String email) throws Exception {
        ...
        try {
            Firestore db = fireBaseConnection.getDB();
            if (!email.isEmpty()) {
                ApiFuture<QuerySnapshot> query = db.collection(COLLECTION_ID).whereEqualTo("email", email).get();
